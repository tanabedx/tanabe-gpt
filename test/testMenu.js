...
function runTestCommand(args) {
  ...
  const testProcess = spawn('node', ['--no-deprecation', path.join(__dirname, 'runTests.js'), ...args], {
    stdio: 'inherit',
    env: env
  });
...
function showCategorySelectionMenu() {
  console.clear();
  console.log('\n=== Select Test Categories ===');
  console.log('Enter the numbers of the categories you want to run tests for, separated by spaces.');
...
function runCategoriesSequentially(categories, index = 0) {
  ...
  testProcess.on('close', (code) => {
    ...
    if (index < categories.length - 1) {
      rl.question(`\nContinue to next category (${categories[index + 1]})? (Y/n): `, (answer) => {
        if (answer.trim().toLowerCase() === 'n') {
          showMainMenu();
        } else {
