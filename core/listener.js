            ...
            try {
                ...
                try {
                    ...
                    if (isWhitelistedPhone && !chat.isGroup) {
                        ...
                        const userState = getUserState(contactId, chatId);
                        if (userState && userState.state !== 'INITIAL') {
                            logger.debug(
                                'Whitelisted user in wizard mode, handling wizard without prefix',
                                { contactId, chatId, state: userState.state }
                            );
                    ...
                    if (userState && userState.state !== 'INITIAL') {
                        logger.debug('User in wizard mode, handling wizard', {
                            userId,
                            chatId,
                    ...
                    if (message.body.includes('@')) {
                        ...
                        const potentialTags = message.body
                            .split(/\s+/)
                        ...
                        if (potentialTags.length > 0) {
                            ...
                            const { isValidTag, validTag } = await commandManager.checkValidTag(
                                potentialTags,
                                chat
                            );
                            ...
                            if (isValidTag) {
                                ...
                                logger.debug(
                                    'No valid tags found in message, continuing to NLP processing'
                                );
                            }
                        }
                    }
        ...
        client.on('message_reaction', async reaction => {
            ...
            try {
                ...
                if (reaction.reaction !== PRAYER_EMOJI) {
                    logger.debug(
                        `Ignoring reaction with emoji ${reaction.reaction} - only ${PRAYER_EMOJI} triggers deletion`
                    );
                    return;
                }
                ...
                if (reaction.msgId.fromMe) {
                    logger.debug(
                        'Message was from bot and has prayer emoji reaction, getting chat'
                    );
                    ...
                    const message = messages.find(
                        msg => msg.id._serialized === reaction.msgId._serialized
                    );
